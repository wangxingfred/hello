...
pub(crate) fn ftb_init(s: &mut Scene, fight_tid: i64, atk_team: &TeamInfo, def_team: &TeamInfo) {
    ...
    if let Cfg::FightCfg(fight_cfg) = &*tb::tb_get(Tab::FightCfg, fight_tid) {
        ...
        if let Cfg::FightTypeCfg(type_cfg) = &*tb::tb_get(Tab::FightTypeCfg, fight_type) {
            ...
            if type_cfg.star_repress || type_cfg.level_repress {
                let repress_list = tb::tb_all(Tab::FightRepressTypeCfg);
...
pub(crate) fn ftb_insert(s: &mut Scene, tab: Tab, tab_key: i32, tid: i64) {
    let cfg = tb::tb_get(tab, tid);
    insert_record(s, cfg, tab_key, tid);
}

pub(crate) fn ftb_get(s: &Scene, tab_key: i32, tid: i64) -> &MapValue {
    let id = (tid << TID_BIT) | (tab_key << TYPE_BIT);
    s.get(id)
}

pub(crate) fn ftb_get_cfg(s: &Scene, tab_key: i32, tid: i64) -> &Cfg {
    if let &MapValue::Cfg(cfg) = ftb_get(s, tab_key, tid) {
        return &*cfg;
    } else {
        panic!(format!("no cfg : tab_key={:?}, tid=={:?}", tab_key, tid))
    }
}
...
fn insert_ability(s: &mut Scene, tid: i64) {
    let cfg = tb::tb_get(tb::Tab::AbilityCfg, tid);
    if let Cfg::Ability(ability) = cfg.borrow() {

        // [insert(?fight_point_cfg, ?FIGHT_POINT_CTX, Point) || Point <- PointL],
        // [insert(?target_rule_cfg, ?TARGET_RULE_CTX, LimitTarget) || LimitTarget <- LimitTargetL],
        // [insert(?target_rule_cfg, ?TARGET_RULE_CTX, EffectTarget) || EffectTarget <- EffectTargetL],
...
fn insert_limit_rule(s: &mut Scene, tid: i64) {
    let cfg = tb::tb_get(tb::Tab::LimitRuleCfg, tid);
    if let Cfg::LimitRuleCfg(rule) = cfg.borrow() {
...
fn insert_ability_effect(s: &mut Scene, tid: i64) {
    let cfg = tb::tb_get(tb::Tab::AbilityEffectCfg, tid);
    if let Cfg::AbilityEffectCfg(ab_effect) = cfg.borrow() {
        ftb_insert(s, Tab::AbilityEffectTypeCfg, ABILITY_EFFECT_TYPE_CTX, ab_effect.type_);
...
fn insert_buff(s: &mut Scene, buff_tid: i64) {
    let cfg = tb::tb_get(Tab::FightBuffCfg, buff_tid);
    if let Cfg::FightBuffCfg(buff_cfg) = &*cfg {
        // insert(?fight_point_cfg, ?FIGHT_POINT_CTX, Point),
...
fn insert_buff_effect(s: &mut Scene, eff_tid: i64) {
    let cfg = tb::tb_get(Tab::FightBuffEffectCfg, eff_tid);
...
fn init_target_rules(s: &mut Scene) {
    let rules = tb::tb_all(Tab::TargetRuleCfg);
    for cfg in rules {
...
fn init_fight_points(s: &mut Scene) {
    for cfg in tb::tb_all(Tab::FightPointCfg) {
...
fn insert_star_repress(s: &mut Scene) {
    for cfg in tb::tb_all(Tab::FightStarRepressCfg) {
...
fn insert_level_repress(s: &mut Scene) {
    for cfg in tb::tb_all(Tab::FightLevelRepressCfg) {
